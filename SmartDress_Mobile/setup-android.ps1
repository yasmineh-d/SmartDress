...
# Step 1: Run native:install with --skip-php and --no-force to create project skeleton only
Write-Host ""
Write-Host "[1/4] Creating Android project skeleton (--skip-php --no-force)..." -ForegroundColor Yellow
# --no-force is CRITICAL to bypass manifest fetch when --skip-php is used on some environments
php artisan native:install android --skip-php --no-force --no-interaction
...
if (!(Test-Path $ANDROID_DIR)) {
    New-Item -ItemType Directory -Path $ANDROID_DIR -Force | Out-Null
}
$EscapedSdkPath = $SDK_DIR.Replace('\', '\\').Replace(':', '\:')
Set-Content -Path $LOCAL_PROPS -Value "sdk.dir=$EscapedSdkPath"
