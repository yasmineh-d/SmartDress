...
<head>
    ...
    <title>SmartDress - Administration</title>
...
<body class="bg-offwhite font-body text-bark min-h-screen" x-data="adminDashboard">
            ...
            Alpine.data('adminDashboard', () => ({
                ...
                save() {
                    if (this.modalType === 'activity') {
                        ...
                        this.currentActivity.initial = this.currentActivity.name.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
                        if (this.isEditing) {
    ...
    <div class="flex min-h-screen pt-20">
        ...
        <main class="flex-1 bg-offwhite">
            ...
            <header
                ...
                <a href="{{ url('/') }}" class="sd-logo">Smart<span>Dress</span></a>
                <button class="text-bark">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 6h16M4 12h12m-8 6h8" />
                    </svg>
                </button>
            </header>
                ...
                <div class="bg-white rounded-[3rem] shadow-sm border border-tan/10 overflow-hidden">
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                            ...
                            <div x-show="tab === 'dashboard'" class="relative w-full sm:w-48">
                                <select x-model="filter"
                                    ...
                                    <option value="génération">Génération de tenue</option>
                        ...
                        <table class="w-full text-left">
                            <thead class="bg-cream/20 text-tan text-[9px] font-bold uppercase tracking-[0.25em]">
                                <tr>
                                    ...
                                    <th class="px-12 py-5">Action</th>
                                    <th class="px-12 py-5 text-center">Horodatage</th>
                            ...
                            <tbody class="divide-y divide-cream/50">
                                <template x-for="activity in filteredActivities" :key="activity.id">
                                    <tr class="hover:bg-cream/10 transition-colors">
                                        <td class="px-12 py-8">
                                            <div class="flex items-center gap-4">
                                                <div class="w-10 h-10 rounded-2xl bg-tan/20 flex items-center justify-center text-xs font-bold text-moss shadow-inner"
                                                    x-text="activity.initial"></div>
                    ...
                    <div x-show="tab === 'users'" class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead class="bg-cream/20 text-tan text-[9px] font-bold uppercase tracking-[0.25em]">
                                <tr>
                                    <th class="px-12 py-5">Nom / Email</th>
                                    <th class="px-12 py-5">Rôle</th>
                            ...
                            <tbody class="divide-y divide-cream/50">
                                <template x-for="user in filteredUsers" :key="user.id">
                                    <tr class="hover:bg-cream/10 transition-colors">
                                        <td class="px-12 py-8">
                                            <div>
                                                <p class="text-sm font-bold text-bark" x-text="user.name"></p>
                                                <p class="text-xs text-tan" x-text="user.email"></p>
                                            </div>
                                        </td>
                                        <td class="px-12 py-8">
                                            <span
                                        ...
                                        <td class="px-12 py-8 text-center">
                                            <span class="text-[10px] font-bold uppercase"
                                                :class="user.status === 'Actif' ? 'text-moss' : 'text-red-400'"
    ...
    <div x-show="showModal"
        ...
        <div class="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl border border-tan/10"
            ...
            <h2 class="text-3xl font-display font-medium text-bark italic mb-8"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select x-model="currentActivity.action"
                                    ...
                                    <option value="Génération de tenue">Génération de tenue</option>
            ...
            <div class="flex gap-4 mt-10">
                <button @click="showModal = false"
                    class="flex-1 py-4 border border-tan/10 text-tan hover:text-bark font-bold text-[10px] uppercase tracking-widest rounded-full transition-all">Annuler</button>
                <button @click="save()"
                    class="flex-1 py-4 bg-moss text-white font-bold text-[10px] uppercase tracking-widest rounded-full shadow-lg shadow-moss/20 hover:bg-bark transition-all">Enregistrer</button>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white w-full max-w-sm rounded-[3rem] p-10 text-center shadow-2xl border border-tan/10"
            ...
            <div class="flex gap-4">
                ...
                <button @click="deleteItem()"
                    class="flex-1 py-4 bg-red-500 text-white font-bold text-[10px] uppercase tracking-widest rounded-full">Supprimer</button>
            </div>
        </div>
    </div>
